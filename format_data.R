...
# Fix incorrect formatting in Grech
grech <- grech[!grepl("[[:digit:]C]", grech$Systematic_ID),]
...
combined <- merge(grech, peptide_stats, by = "Systematic_ID", all.x = TRUE)

#%>%
  merge(., interactors, by = "Systematic_ID", all.x = TRUE) %>%
