...
```{r}
# Get lengths of chr I and II
chrI_len <- max(grech$start[grech$chrI == 1], na.rm = TRUE)
chrII_len <- max(grech$start[grech$chrII == 1], na.rm = TRUE)

grech$start[grech$chrII == 1] <- grech$start[grech$chrII == 1] + chrI_len
grech$start[grech$chrIII == 1] <- grech$start[grech$chrIII == 1] + chrII_len

grech <- grech %>% select(-end) %>% rename(genomic_location = start)
...
functional_centralities <- get_centralities(functional_interactome) %>% 
  rename(funct_degr_centr = deg_centr, funct_betw_centr = betw_centr,
...
physical_centralities <- get_centralities(physical_interactome) %>% 
  rename(phys_degr_centr = deg_centr, phys_betw_centr = betw_centr,
...
cols_to_ignore <- combined2 %>% select(Systematic_ID, starts_with("Function"),
                                      ...
                                      N, P, Q, R, S, T, V, W, Y, mean.phylop) %>%
  colnames

imputed <- impute_missing(combined2, n_cores=6, random_seed=42,
               cols_to_ignore = cols_to_ignore)


# Fill remaining missing values with 0
imputed <- imputed %>% mutate_all(~replace(., is.na(.), 0))
